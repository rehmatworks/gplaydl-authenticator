...
class AasAuthenticator {
    ...
    suspend fun mint(email: String, oauthToken: String): MintedCredentials =
        withContext(Dispatchers.IO) {
            val form = FormBody.Builder()
                ...
                .add("google_play_services_version", PLAY_SERVICES_VERSION.toString())
                .add("sdk_version", Build.VERSION.SDK_INT.toString())
            ...
            val fields = parseKeyValueBody(body)

            // Google reports failures in the body, not the status code.
            fields["Error"]?.let { throw AasAuthException(describeError(it, fields)) }

            val token = fields["Token"]
                ?: throw AasAuthException("Google did not return an AAS token. Please try signing in again.")
    ...
    private companion object {
        ...
        const val PLAY_SERVICES_VERSION = 19629032
    }
}
